...
transects <- 
  ...
  geom_sf(data=centre, fill = "gray21", colour = NA) +
  geom_sf(data=second, fill = "grey", colour = NA) +
  geom_sf(data=third, fill = "lightgrey", colour = NA) +
  
  # plot the actual survey route
  geom_segment(aes(x = 250, y = 0, xend = 250, yend = 4000),linetype = "dashed", colour = "cyan2", size=0.05) +
  geom_segment(aes(x = 750, y = 0, xend = 750, yend = 4000),linetype = "dashed", colour = "cyan2", size=0.05) +
