...
data.frame(sum(res$late.det),(sum(res$early.det)))
}


# run a fisher exact test on between data generated by paramsA and paramsB
# the fisher test A takes params from the species parameterA, B from B etc.

fish.datA<-function(){
fishA<-ldply(1:sims, function(i) tibble(iteration = i, (surv.prod())))
fishA
}

fish.datB<-function(){
  fishB<-ldply(1:sims, function(i) tibble(surv.prod()))
  fishB
}

# in this one you'd hopefully be able to do something like
# parameters_a<- c(lay.date_a...,)
# parameters_b<- c(lay.date_b...,)

dat.sig.test<-cbind(sig.datA(),sig.datB())
names(dat.sig.test)[2]<-"k"
names(dat.sig.test)[3]<-"n"
names(dat.sig.test)[4]<-"k2"
names(dat.sig.test)[5]<-"n2"
tt
sig.test <- tt %>%   
  group_by(iteration) %>% 
  nest()  %>% 
  mutate(matrix = map(data, ~matrix(unlist(.x), nrow = 2))) %>% 
  mutate(fisher = map(matrix, ~fisher.test(.x, alternative = "less"))) %>% 
...
# mapfisher test
mapmodels<-function(df){
  fisher.test(cbind(c(df$k,df$n-df$k), c(k,n-k)), alternative="less")}

tt<- purrr::map(mod.data2$data, mapmodels)

# when you use map you have to do it on a list

# this is a fisher.test which test for difference between two groups of binary data 
fisher.test(rbind(c(8,22-8), c(14,23-14)), alternative="less")

fisher.test(cbind(c(df$k,df$n-df$k), c(df2$k,df2$n-df2$k)), alternative="less")
#
